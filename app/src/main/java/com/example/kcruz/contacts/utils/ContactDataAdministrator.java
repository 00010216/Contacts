...
public class ContactDataAdministrator {
    public static final ContactDataAdministrator CONTACT_UTILS = new ContactDataAdministrator();
    private ArrayList<Contact> mContacts;
    Context context = MainActivity.getContext();
    int id_gen;

    public ContactDataAdministrator() {
        loadDeviceContacts();
        id_gen = 1;

    }

    public static ContactDataAdministrator getInstance(){
        return CONTACT_UTILS;
    }

    public int generateId(){
        for (int i = 0; i < mContacts.size(); i++){
            if(id_gen == mContacts.get(i).getId()){
                id_gen++;
            }
        }
        return id_gen;
    ...
    private void loadDeviceContacts(){
        ...
        if (cursor != null && cursor.getCount() > 0) {
            while (cursor.moveToNext()) {
                ...
                mContact.setId(Integer.parseInt(cursor.getString(cursor.getColumnIndex(ContactsContract.Contacts._ID))));
                mContact.setFirstName(cursor.getString(cursor.getColumnIndex(ContactsContract.Contacts.DISPLAY_NAME)));
    ...
    private void retrievePhone(ContentResolver contentResolver) {
        ...
        mContact.setNumber1(phoneNumber);
        cursor.close();
    }
